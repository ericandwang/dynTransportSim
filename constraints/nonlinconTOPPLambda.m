function [c, ceq, dc, dceq] = nonlinconTOPPLambda(P, s0, ss, param, fCone, vec, tol, xp, dxp, ddxp, yp, dyp, ddyp, dcFun, dceqFun, accelLim, tt)

% separating P and lambda
lambda = P(end);
P = P(1:end-1);
...
for j = 1:nPoints
    for i = 1:dim
        c(i + dim*(j-1)) = dot(vec(:,i),p(:,j)-fCone(:,i)) + norm(vec(:,i))*tol + norm(vec(:,i))*lambda;
    end
end
...
% appending transit time constraint
cappend = sum(1./(P(1:nPoints-1,1)+P(2:nPoints,1))) - tt;
c = [c; cappend];
...
if nargout > 2
dc = dcFun(Pmat);
dc = [dc; [repmat(vecnorm(vec),1,nPoints), zeros(1,nPoints)]];
gradf = zeros(size(Pmat));
gradf(1,1) = -1/(P(1,1) + P(2,1))^2;
gradf(nPoints,1) = -1/(P(nPoints-1,1) + P(nPoints,1))^2;
gradf(2:nPoints-1,1) = -1./(P(1:nPoints-2,1)+P(2:nPoints-1,1)).^2 - 1./(P(2:nPoints-1,1)+P(3:nPoints,1)).^2;
dc = [dc, [gradf; 0]];

% equality gradients
dceq = dceqFun(Pmat);
dceq = [dceq; zeros(1,size(dceq,2))];
end
